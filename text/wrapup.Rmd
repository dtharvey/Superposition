...
You can use the download buttons on the upper right to retrieve the data used in this module to demonstrate the simulation of a square wave, a triangular wave, and a sawtooth wave using the superposition of sine waves. Each file is in a csv format and has 15 columns. Each column gives the value of one sine wave, $y(x)$, in the Fourier series from $x = 0$ to $x = 4\pi$ in intervals of $0.01\pi$. The first column is for the sine wave where $k = 1$ and the last column is for the sine wave where $k = 15$.

#### Visuallizing a Fourier Series

In this module we used two ways to display the Fourier series for a square wave, a triangular wave, and a sawtooth wave: an overlay of the individual sine waves in the Fourier series and the equation that describes the Fourier series. Both have limitations. Overlaying individual sine waves results in a cluttered plot in which it is difficult to discern between most sine waves beyond the first three. The equation for a Fourier series tells us how the amplitude and the periodicity change from wave-to-wave---for a square wave, each successive sine wave has an amplitude that decreases by a factor of $2k - 1$ and a periodicity that increases by a factor of $2k - 1$---but not in way that is easy to visualize.

The plot on the right provides a more useful way to visualize the Fourier series for a square wave. Each vertical line gives the amplitude, $A$, and the periodicity, $a$, for one of the first 15 terms in the square wave's Fourier series, with the first member of the series $k = 1$, on the far left. The position of the lines along the *x*-axis shows that only sine waves with an odd value of $a$ contribute to the square wave's Fourier series. Values of the amplitude are displayed above each vertical line.

#### Sources of Waves

Our treatment of the superposition of waves did not consider the source of these waves. Although in analytical chemistry we often think of waves in terms of electromagnetic radiation, the superposition of waves applies to any type of periodic disturbance in a medium; sound waves, for example, are a common example, as are waves moving through water.
